...
function drawText() {
  ...
  if (frameNumber < 300) {
    ...
    context.fillText(
      "I've been thinking it would be great to spend some time together.",
      canvas.width / 2,
      canvas.height / 2
    );
  ...
  if (frameNumber > 1200 && frameNumber < 1500) {
    context.fillStyle = `rgba(255, 255, 255, ${opacity})`;
    context.fillText(
      "it would be lovely to chat and get to know each other better.",
      canvas.width / 2,
      canvas.height / 2
    );
    opacity = opacity + 0.01;
  ...
  if (frameNumber > 3000 && frameNumber < 99999) {
    ...
    if (window.innerWidth < 600) {
      drawTextWithLineBreaks(
        [
          "Madam , to aap apna Calendar mei ek achi si date dekh ke appointment de do",
        ],
        canvas.width / 2,
        canvas.height / 2,
        fontSize,
